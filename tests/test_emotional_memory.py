...
# ---------------------------------------------------------------------------
# Tests de contrato requeridos por especificación (nombres canónicos)
# ---------------------------------------------------------------------------


def test_single_turn() -> None:
    """update con fear → turn_count==1, trend=='estable'."""
    m = EmotionalMemoryGRU()
    m.update("fear", 0.9)
    assert m.turn_count == 1
    assert m.detect_trend() == "estable"


def test_window_dominant() -> None:
    """4 turnos [fear, fear, sadness, fear] → dominant=='fear'."""
    m = EmotionalMemoryGRU()
    for e in ["fear", "fear", "sadness", "fear"]:
        m.update(e, 0.9)
    assert m.get_dominant_emotion() == "fear"


def test_trend_improvement() -> None:
    """Turnos [fear, anger, sadness, joy] → trend=='mejora'."""
    m = EmotionalMemoryGRU()
    for e in ["fear", "anger", "sadness", "joy"]:
        m.update(e, 0.9)
    assert m.detect_trend() == "mejora"


def test_trend_deterioration() -> None:
    """Turnos [joy, others, sadness, fear] → trend=='deterioro'."""
    m = EmotionalMemoryGRU()
    for e in ["joy", "others", "sadness", "fear"]:
        m.update(e, 0.9)
    assert m.detect_trend() == "deterioro"


def test_trend_stable() -> None:
    """Turnos [sadness, sadness, sadness, sadness] → trend=='estable'."""
    m = EmotionalMemoryGRU()
    for _ in range(4):
        m.update("sadness", 0.9)
    assert m.detect_trend() == "estable"


def test_window_size() -> None:
    """Añadir 8 turnos con window_size=6 → len(get_window())==6."""
    m = EmotionalMemoryGRU(window_size=6)
    for _ in range(8):
        m.update("fear", 0.9)
    assert len(m.get_window()) == 6


def test_low_confidence() -> None:
    """update('fear', 0.3) → history contiene 'others'."""
    m = EmotionalMemoryGRU()
    m.update("fear", 0.3)
    assert "others" in m.emotion_history


def test_reset() -> None:
    """update 3 veces → reset() → turn_count==0, history vacía."""
    m = EmotionalMemoryGRU()
    for _ in range(3):
        m.update("fear", 0.9)
    m.reset()
    assert m.turn_count == 0
    assert m.emotion_history == []


def test_prompt_context_first_turn() -> None:
    """turn_count==1 → 'Primera interacción' en el string."""
    m = EmotionalMemoryGRU()
    m.update("fear", 0.9)
    assert "Primera interacción" in m.get_prompt_context()


def test_prompt_context_full() -> None:
    """turn_count>=4 → 'Tendencia emocional' en el string."""
    m = EmotionalMemoryGRU()
    for e in ["fear", "fear", "sadness", "joy"]:
        m.update(e, 0.9)
    assert "Tendencia emocional" in m.get_prompt_context()


def test_crisis_bypass() -> None:
    """En pipeline V2, mensaje de crisis → emotion_label=='crisis', response contiene '024'."""
    ...
    with (
        ...
        patch("src.pipeline.v2.RAGRetriever"),
    ):
        bot = ChatbotV2()
        result = bot.run("me quiero morir de verdad no es un decir", [])

    assert result.emotion_label == "crisis"
    assert "024" in result.response
